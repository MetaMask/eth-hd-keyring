...
import { HDKeyring, KeyringOpt } from './HDKeyring';
...
/* Jest can't await the constructor, and when the error gets thrown, Jest can't catch it. */
/* This function is a workaround to test the constructor errors.
 */
const testConstructorError = (
  keyringOpts: KeyringOpt,
  expectedError: HDKeyringErrors,
) => {
  try {
    () => new HDKeyring(keyringOpts);
  } catch (e: any) {
...
describe('hd-keyring', () => {
  ...
  describe('re-initialization protection', () => {
    it('double generateRandomMnemonic', () => {
      ...
      expect(() => {
        keyring.generateRandomMnemonic();
      }).toThrow(HDKeyringErrors.SRPAlreadyProvided);
    });
    ...
    it('constructor + deserialize', async () => {
      const keyring = new HDKeyring({
        mnemonic: sampleMnemonic,
        numberOfAccounts: 2,
      });
      ...
      ).rejects.toThrow(HDKeyringErrors.SRPAlreadyProvided);
    });
  });
  ...
  describe('#serialize mnemonic.', () => {
    ...
    it('throws if mnemnoic is not set', async () => {
      ...
      await expect(keyring.serialize()).rejects.toThrow(
        HDKeyringErrors.MissingMnemonic,
      );
    });
  });
  ...
  */
  ...
  describe('#signMessage', function () {
    ...
    it('reliably can decode messages it signs', async function () {
      ...
      signatures.forEach((sgn, index) => {
        ...
        const rValue = toBuffer(sgn.slice(0, 66));
        const sValue = toBuffer(`0x${sgn.slice(66, 130)}`);
        const vValue = BigInt(`0x${sgn.slice(130, 132)}`);
        const messageBuffer = toBuffer(msgHashHex);
        const pub = ecrecover(messageBuffer, vValue, rValue, sValue);
    ...
    it('throw error for invalid message', async function () {
      ...
      await expect(keyring.signMessage(firstAcct, '')).rejects.toThrow(
        HDKeyringErrors.InvalidMessage,
      );
    });
    ...
    it('throw error if address not associated with the current keyring is passed', async function () {
      ...
      ).rejects.toThrow(HDKeyringErrors.NoMatchingAddress);
    });
  });
  ...
  describe('exportAccount', function () {
    ...
    it('throw error if account is not present', async function () {
      await expect(keyring.exportAccount(notKeyringAddress)).rejects.toThrow(
        HDKeyringErrors.AddressNotFound,
      );
    });
  });
  ...
  describe('#encryptionPublicKey', function () {
    ...
    it('throw error if address is not present in the keyring', async function () {
      ...
      ).rejects.toThrow(HDKeyringErrors.AddressNotFound);
    });
  });
